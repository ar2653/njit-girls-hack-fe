<template>
  <div>
    <audio loop style="display: none" ref="player">
      <source
        src="../../public/assets/audio/interstellar_piano.mp3"
        type="audio/mpeg"
      />
    </audio>
  </div>
  ...
  <PlanetCard
    ...
    @closeCard="selectedPlanetCard = null"
  />
  <div v-if="false" class="date-display" :class="{ disabled: idealizedSpeed }">
    <div class="ico">
    ...
    createSolarSystem: async function (scene) {
      ...
      for (let planet of PLANETS) {
        ...
        if (planet.orbitObject != null) {
          ...
          gltf.scene.rotation.z = THREE.MathUtils.degToRad(
            planet.axialTilt ?? 0
          );
          ...
          pivot.rotation.x = THREE.MathUtils.degToRad(
          ...
          let trajectory = new THREE.Mesh(
            new THREE.TorusGeometry(planet.scaledOrbitalRadius, 0.05, 8, 64),
            material
          );
      ...
      planet.tick = (e) => {
        ...
        if (planet.userData.orbitalRadius !== 0) {
          planet.userData.currentDistance += this.idealizedSpeed
            ? Math.max(
                (e *
                  planet.userData.orbitalVelocity *
                  planet.userData.orbitalRadius) /
                  100,
                6000
              )
          ...
          if (
            planet.userData.currentDistance >
            planet.userData.orbitalCircumference
          ) {
            planet.userData.currentDistance =
              planet.userData.currentDistance %
              planet.userData.orbitalCircumference;
        ...
        let rY =
          (planet.userData.currentRotation /
            planet.userData.planetCircumference) *
          Math.PI *
          2;
